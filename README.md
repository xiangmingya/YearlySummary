...
一款功能强大的 Typecho 年度文章统计插件，帮助博主全面了解自己的写作数据，支持多维度统计分析、数据可视化和导出功能。
...
### 图表展示
![图表展示](screenshots/charts.png)

### 排行榜
![排行榜](screenshots/ranking.png)
...
1. 在统计页面右上角，点击「导出 JSON」或「导出 CSV」按钮
2. 浏览器会自动下载对应格式的统计数据文件
...
| 自动检查更新 | 是否自动检查插件更新 | 开启 |
| GitHub 仓库 | 设置插件的 GitHub 仓库地址 | xiangmingya/YearlySummary |
...
- Typecho 1.2 或更高版本
- PHP 7.4 或更高版本
- 支持 `file_get_contents` 函数（用于更新检查）

## 注意事项

1. **浏览量统计**：浏览量数据需要安装支持 `views` 字段的浏览统计插件（如 ViewsCounter），否则浏览量相关数据将显示为 0。

2. **更新检查**：更新检查功能需要服务器能够访问 GitHub API，如果服务器网络受限，可以在插件设置中关闭此功能。

3. **数据准确性**：统计数据基于文章的发布时间（created 字段），修改文章不会影响其所属年份。
...
A: 在插件设置中，将「自动检查更新」选项设置为「关闭」即可。

### Q: 导出的 CSV 文件用 Excel 打开乱码？

A: 插件导出的 CSV 文件已添加 BOM 头，正常情况下 Excel 可以正确识别。如果仍有乱码，请尝试用记事本打开后另存为 UTF-8 编码。
